        ...
        <section class="section">
          <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
              ...
              <div class="breadcrumb-item">Data User Admin</div>
            </div>
          </div>

            <?php echo $this->session->userdata('message1') <> '' ? $this->session->userdata('message1') : ''; ?>
            <div class="text-center">
                <button class="btn btn-primary" onclick="add_user_admin()" data-toggle="tooltip" data-placement="bottom" title="Tambah">
            ...
            <script type="text/javascript">
              ...
              $(document).ready(function() {
                  ...
                  table = $('#table').DataTable({ 
                      ...
                      "ajax": {
                          "url": "<?php echo site_url('user_admin/ajax_list')?>",
                          "type": "POST"
                      },
              ...
              function edit_user_admin(id)
              {
                  ...
                  $.ajax({
                      url : "<?php echo site_url('user_admin/ajax_edit')?>/" + id,
                      type: "GET",
                      dataType: "JSON",
              ...
              function delete_user_admin(id)
              {
                  if(confirm('Apakah anda yakin akan menghapus data ini?'))
                  {
                      ...
                      $.ajax({
                          url : "<?php echo site_url('user_admin/ajax_delete')?>/"+id,
                          type: "POST",
                          dataType: "JSON",
                          success: function(data)
                          {
                              //if success reload ajax table
                              document.location = "<?php echo base_url('user_admin')?>";
                          },
                          error: function (jqXHR, textStatus, errorThrown)
                          {
                  ...
                  if(list_id.length > 0)
                  {
                      if(confirm('Apakah anda yakin akan menghapus '+list_id.length+' data ini?'))
                      {
                          $.ajax({
                              type: "POST",
                              data: {id:list_id},
                              url: "<?php echo site_url('user_admin/ajax_bulk_delete')?>",
                              dataType: "JSON",
                              success: function(data)
                              {
                                  if(data.status)
              ...
              function save()
              {
                  ...
                  if(save_method == 'add') {
                      url = "<?php echo site_url('user_admin/ajax_add')?>";
                  } else {
                      url = "<?php echo site_url('user_admin/ajax_update')?>";
                  }
